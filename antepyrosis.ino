/***
 * Antepyrosis source code for caledra.sydney
 * Designed to run on Arduino UNO
 * 
 * Pinout:
 *   5 RX of MP3 Player Chipset
 *   6 TX of MP3 Player Chipset
 *   12 Trigger pin of Ultrasonic Sensor
 *   11 Echo pin of Ultrasonic Sensor
 *   3 LED Data Pin for WS2812b strip
 * 
 */
#include <NewPing.h>
...
void setup() {
  ...
  LEDS.addLeds<WS2812, LED_DT, BGR>(leds, NUM_LEDS);
...
void loop() {
  EVERY_N_MILLISECONDS(500) {
    ...
    unsigned long sonar_ping = sonar.ping_cm();
    if (sonar_ping > 10 && sonar_ping < 120) {
      if (playMode != 0) {
          Serial.println("close mode");
        sendMP3Command('1');
        playMode = 0;
        for (int in = 255; in > 150; in -= 5) {
                  max_bright = in;
              FastLED.setBrightness(max_bright);
        }
        baseC = 359;
    ...
    } else if (sonar_ping > 10) {
      if (playMode != 1){
        Serial.println("away mode");
        sendMP3Command('2');
        playMode = 1;
        for (int in = 150; in < 255; in += 5) {
          max_bright = in;
          FastLED.setBrightness(max_bright);
        }
        baseC = 100;
  ...
  EVERY_N_SECONDS(5) { // Change the target palette to a random one every 5 seconds.
     updateColors();   // You can use this as a baseline colour if you want similar hues in the next line.
  }
...
/** This code is modified from the FastLED Plasma Demo **/
...
/** This code is taken from the MP3 Player Module Documentation (ie only code examples and no full documentation) */
